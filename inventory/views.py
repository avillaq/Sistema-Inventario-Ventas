...
def _filtered_products(query, low_stock_only=False):
...
def inventory_overview(request):
    query, low_stock_only = _extract_filters(request)
    products = _filtered_products(query, low_stock_only)
    ...
    context = {
        ...
        "filters": {"q": query, "low": "1" if low_stock_only else ""},
        "form": ProductForm(),
        "current_q": query,
...
def _form_context(*, form, query, low_stock_only, title, submit_label, product=None, message=None):
    return {
        "form": form,
        "form_title": title,
...
def _table_context(query, low_stock_only):
    products = _filtered_products(query, low_stock_only)
    return {
        "products": products.order_by("name")[:200],
        "product_count": products.count(),
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            context = {
                ...
                **_table_context(query, low_stock_only),
            }
